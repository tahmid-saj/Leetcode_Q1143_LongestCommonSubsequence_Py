class Solution:
    ...
    def recursive(self, text1, text2, index1, index2):
        # base case: index1 == index2 and index1 < 0: return 0
        ...
        if text1[index1] == text2[index2]:
            res1 = 1 + self.recursive(text1, text2, index1 - 1, index2 - 1)
        else:
            res2 = self.recursive(text1, text2, index1 - 1, index2)
            res3 = self.recursive(text1, text2, index1, index2 - 1)
    ...
    def bottomUpLinearSpace(self, text1, text2):
        ...
        for i in range(len(text2) - 1, -1, -1):
            ...
            for j in range(len(text1) - 1, -1, -1):
                if text2[i] == text1[j]: curr[j] = prev[j + 1] + 1
